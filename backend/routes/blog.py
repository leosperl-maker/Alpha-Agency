...
async def get_pending_comments(user = Depends(get_current_user)):
    """Get all pending comments across all articles (Admin only)"""
...
async def moderate_comment(
    ...
    """Approve, reject, or mark comment as spam (Admin only)"""
    if moderation.status not in ["approved", "rejected", "spam"]:
        raise HTTPException(status_code=400, detail="Statut invalide")
    
    # Find article containing this comment
    article = await db.blog_posts.find_one(
    ...
    return {
        "success": True,
        "message": f"Commentaire {status_text.get(moderation.status)}"
    }
