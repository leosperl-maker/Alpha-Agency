from fastapi import FastAPI, APIRouter, HTTPException, Depends, Request, BackgroundTasks, File, UploadFile, Form, Body
...
# NOTE: Routes /invoices sont maintenant dans routes/invoices.py
...
class TagSuggestionRequest(BaseModel):
    content: str
    title: Optional[str] = None
    type: str = "portfolio"  # portfolio or blog

@api_router.post("/tags/suggest", response_model=dict)
...
{content_preview}"""
    ...
    try:
            ...
            if json_match:
                try:
                    ...
                    return {
                        ...
                        "new_tags": list(set(truly_new))[:3],  # Max 3 new tags
        ...
        logging.error(f"Error suggesting tags: {str(e)}")
...
async def sync_meta_comments(account: dict, user_id: str) -> int:
    ...
    # Check if this is a Facebook page or Instagram business account
    account_type = account.get("account_type", "")
    if account_type not in ["page", "facebook_page", "instagram_business"]:
        return 0
    ...
    async with httpx.AsyncClient() as client:
        try:
            if account_type == "instagram_business":
                # Instagram: Get recent media with comments
                response = await client.get(
                    f"https://graph.facebook.com/v20.0/{page_id}/media",
                    params={
                        "fields": "id,caption,timestamp,comments.limit(25){id,from,text,timestamp}",
                        "limit": 10,
                ...
                if response.status_code == 200:
                    ...
                    for media in data.get("data", []):
                        ...
                        media_caption = (media.get("caption") or "")[:100]
                        ...
                        for comment in media.get("comments", {}).get("data", []):
                            ...
                            if not existing:
                                ...
                                msg = {
                                    ...
                                    "post_id": media_id,
                                    "post_content": media_caption,
                                    "status": "unread",
                                    "priority": "normal",
                                    "created_at": comment.get("timestamp", datetime.now(timezone.utc).isoformat()),
                                    "updated_at": datetime.now(timezone.utc).isoformat()
                                }
                                await db.social_messages.insert_one(msg)
                                new_count += 1
            else:
                # Facebook: Get recent posts with comments
                response = await client.get(
            ...
            logging.error(f"Error syncing {platform} page {page_id}: {e}")
...
async def forgot_password(request: PasswordResetRequest, background_tasks: BackgroundTasks):
    ...
    # Send reset email
    frontend_url = os.environ.get('FRONTEND_URL', 'https://meta-oauth-upgrade.preview.emergentagent.com')
...
async def exchange_meta_token(data: MetaTokenExchange, current_user: dict = Depends(get_current_user)):
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            # Store Meta credentials in database
            meta_account = {
                "id": str(uuid.uuid4()),
                "user_id": current_user["user_id"],
...
@api_router.get("/meta/pages", response_model=List[dict])
...
@api_router.post("/meta/publish/facebook", response_model=dict)
...
@api_router.post("/meta/publish/instagram", response_model=dict)
...
@api_router.get("/meta/published-posts", response_model=List[dict])
...
@api_router.delete("/meta/disconnect", response_model=dict)
