...
const categoryColors = {
  // Standard news categories
  general: { bg: "bg-secondary", text: "text-foreground", accent: "#6B7280" },
  business: { bg: "bg-info-soft", text: "text-info", accent: "#3B82F6" },
  technology: { bg: "bg-brand-soft", text: "text-primary", accent: "#8B5CF6" },
  science: { bg: "bg-success-soft", text: "text-emerald-700", accent: "#10B981" },
  health: { bg: "bg-danger-soft", text: "text-danger", accent: "#EF4444" },
  sports: { bg: "bg-warning-soft", text: "text-warning", accent: "#F59E0B" },
  entertainment: { bg: "bg-pink-100", text: "text-pink-700", accent: "#EC4899" },
  // Marketing categories
  ads: { bg: "bg-warning-soft", text: "text-warning", accent: "#F97316" },
  social: { bg: "bg-info-soft", text: "text-cyan-700", accent: "#06B6D4" },
  growth: { bg: "bg-lime-100", text: "text-lime-700", accent: "#84CC16" },
  crm: { bg: "bg-violet-100", text: "text-violet-700", accent: "#A855F7" },
  local: { bg: "bg-success-soft", text: "text-success", accent: "#22C55E" },
  design: { bg: "bg-danger-soft", text: "text-rose-700", accent: "#E11D48" },
};
...
const FeaturedArticle = ({ article, onClick }) => {
  ...
  return (
    <Card 
      className="bg-card backdrop-blur-xl border-border overflow-hidden cursor-pointer hover:shadow-xl transition-all duration-300 group"
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
        ...
        <div className="p-6 lg:p-8 flex flex-col justify-center order-2 lg:order-1">
          ...
          <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-foreground mb-4 group-hover:text-primary transition-colors line-clamp-3">
            {article.title}
          </h2>
        ...
        <div className="relative aspect-video lg:aspect-auto lg:h-full min-h-[200px] lg:min-h-[300px] order-1 lg:order-2">
          {article.image_url ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-[#CE0202] to-[#8B0000] flex items-center justify-center">
              <Newspaper className="w-16 h-16 text-foreground opacity-50" />
            </div>
          )}
...
const ArticleCard = ({ article, onClick, onDelete }) => {
  ...
  return (
    <Card 
      className="bg-card backdrop-blur-xl border-border overflow-hidden cursor-pointer hover:shadow-lg transition-all duration-300 group h-full flex flex-col"
    ...
    >
      ...
      >
        {article.image_url ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-[#CE0202]/80 to-[#8B0000] flex items-center justify-center">
            <Icon className="w-10 h-10 text-foreground opacity-70" />
          </div>
        )}
      ...
      >
        ...
        <p className="text-muted-foreground text-xs sm:text-sm leading-relaxed line-clamp-2 mb-3 flex-1">
          {article.description}
        </p>
        
        {/* Footer */}
        <div className="flex items-center justify-between text-xs text-muted-foreground pt-2 border-t border-[#F5F5F5]">
...
const ArticleDetailModal = ({ article, relatedArticles, onClose, onArticleClick }) => {
  ...
  return (
    <div 
      className="fixed inset-0 z-50 bg-card backdrop-blur-sm flex items-start justify-center overflow-y-auto"
      onClick={onClose}
      data-testid="article-modal"
    >
      <div 
        className="relative w-full max-w-4xl bg-card backdrop-blur-xl min-h-screen lg:min-h-0 lg:my-8 lg:rounded-2xl lg:shadow-2xl overflow-hidden"
      ...
      >
        ...
        <Button 
          variant="ghost" 
          size="sm" 
          className="absolute top-4 right-4 z-10 h-10 w-10 p-0 bg-card backdrop-blur-xl/90 hover:bg-secondary backdrop-blur-xl rounded-full shadow-lg"
          onClick={onClose}
        ...
        <Button 
          variant="ghost" 
          size="sm" 
          className="absolute top-4 left-4 z-10 bg-card backdrop-blur-xl/90 hover:bg-secondary backdrop-blur-xl rounded-full shadow-lg"
          onClick={onClose}
        >
        ...
        <div className="relative w-full aspect-video lg:aspect-[21/9]">
          {article.image_url ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-[#CE0202] to-[#8B0000] flex items-center justify-center">
              <Icon className="w-20 h-20 text-foreground opacity-50" />
            </div>
          )}
        </div>
        ...
        <div className="p-6 lg:p-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-8 pb-6 border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="font-medium text-foreground">{article.source_name}</p>
                <p className="text-xs text-muted-foreground">Source originale</p>
              </div>
            </div>
            <a 
            ...
            >
              <Button variant="outline" size="sm" className="text-primary border-primary/40 hover:bg-primary hover:text-foreground">
            ...
            <div className="mt-10 pt-8 border-t border-border">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {relatedArticles.map((related) => (
                  <Card 
                    key={related.id}
                    className="bg-card border-0 overflow-hidden cursor-pointer hover:shadow-md transition-all group"
                  ...
                  >
                    <div className="aspect-video relative">
                      {related.image_url ? (
                        ...
                        <div className="w-full h-full bg-gradient-to-br from-[#CE0202]/60 to-[#8B0000] flex items-center justify-center">
                          <Newspaper className="w-6 h-6 text-foreground opacity-70" />
                        </div>
                      )}
                    </div>
                    <CardContent className="p-3">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">{related.source_name}</p>
                    </CardContent>
                  </Card>
                ))}
              </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Select value={selectedRegion} onValueChange={setSelectedRegion}>
            <SelectTrigger className="w-full sm:w-40 bg-card backdrop-blur-xl border-border">
      ...
      {loading ? (
        ...
        <Card className="bg-card backdrop-blur-xl border-border">
          <CardContent className="py-16 text-center">
            ...
            <h3 className="text-xl font-semibold text-foreground mb-2">Aucune actualité</h3>
